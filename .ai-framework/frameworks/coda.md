...
**Example:**
> "I see we need to add filtering to the locations page (context). The goal is to help users find specific services quickly (objective). I'm thinking a dropdown with our 6 service categories, filtering the cards client-side with a smooth animation (details). We'll verify it works across mobile/desktop and matches our design system (acceptance). Sound good?"
...
**"Think twice, code once"** - structured thinking before implementation:

- **Catches issues early**: Problems discovered in thinking, not after coding
...
- **Efficient communication**: Skip the document bureaucracy while keeping the critical thinking

## Example: CODA in Action

❌ **Without CODA thinking:**
```
User: "Add filtering to the locations page"
AI: [Immediately codes a solution]
User: "That doesn't match our design system"
AI: [Refactors]
User: "The interaction feels wrong"
AI: [Adjusts]
...
✅ **With CODA thinking (conversational):**
```
User: "Add filtering to the locations page"
AI: "Looking at the locations page - I see you have service categories in the data model.
     I'm thinking we add a filter dropdown that shows all 6 services, with 'All Locations'
     as default. Clicking a category filters cards client-side with your naturalEase animation.
     Mobile-first design, matches the existing glassmorphism cards. Want to see 5 variations
     first or shall I implement this approach?"
User: "Perfect, go ahead"
AI: [Implements once, correctly]
Result: All critical thinking up-front, clean implementation
```

✅ **With heavy CODA (formal document):**
```
User: "Add filtering - write me a full plan first"
AI: [Writes detailed CODA document covering approach, design, responsive, animation]
User: "The animation seems aggressive - tone it down"
AI: [Updates plan with subtler animation specs]
User: "Perfect, let's proceed"
AI: [Implements once, correctly]
Result: Documented decision-making, implementation is clean
```

## Integration with Standard Workflow

**Default workflow (Light CODA - conversational):**

1. **Think** through the problem using CODA dimensions
2. **Research** codebase for context (existing patterns, design system, constraints)
3. **Present** your understanding conversationally: "Here's what I see... here's my approach..."
4. **Verify** understanding aligns before implementing
5. **Implement** once alignment is achieved
6. **Review** what was done and verify acceptance criteria

**When user requests formal plan (Heavy CODA):**

1. Think through the problem, read codebase for context
2. **Write CODA plan** (use template below)
3. **Present plan** and get user approval
...
❌ Jumping to implementation without thinking through CODA dimensions
❌ Writing formal CODA documents when conversational approach would work
❌ Skipping CODA thinking for "obviously simple" tasks that turn complex
❌ Using vague language ("modern", "better", "nicer", "cleaner")
❌ Forgetting accessibility, performance, or security considerations
❌ Not getting feedback before implementing (CODA's value is in alignment!)
❌ Treating CODA as documentation bureaucracy instead of critical thinking tool
