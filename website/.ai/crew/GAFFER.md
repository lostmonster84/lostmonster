...
**Design Guide:** [.ai/LOST-MONSTER-DESIGN-SYSTEM.md](../LOST-MONSTER-DESIGN-SYSTEM.md)
...
| **The Gaffer** | **Checks that SOFAX scored honestly, TERRX tested thoroughly, NIGELX was actually consulted, AIDAX wasn't skipped, and PIXLX covered the edge cases** |
...
```
                    ┌──────────┐
                    │   THE    │
                    │  GAFFER  │  ← Manages all workers, tracks everything
                    └────┬─────┘
                         │
          ┌──────────────┼──────────────┐
          │              │              │
...
Lost Monster is a **single Next.js marketing site** — not a monorepo, not a SaaS platform. It is a personal brand website for a developer who runs businesses.

**Key facts for routing and review:**

| Attribute | Value |
|-----------|-------|
| **Domain** | Personal brand development agency |
| **Structure** | Single Next.js app (App Router) |
| **Public pages** | `app/` |
| **Admin** | N/A (no admin dashboard — future consideration) |
| **API routes** | `app/api/` |
| **Database** | Neon PostgreSQL (or TBD) |
| **Hosting** | Vercel |
| **Design system** | Bold Personal Brand — dark gradients, dynamic accent colour (5 options), massive typography, glassmorphism |
| **Voice** | First person ("I"), direct, no corporate speak |
| **Test persona** | Dave, 42, small business owner, not technical, wants proof not jargon |
| **Primary entities** | Projects (portfolio work) |
| **Secondary entities** | Services (what Lost Monster offers) |
| **Tertiary entities** | Testimonials (client proof) |

**Design Guide:** [.ai/LOST-MONSTER-DESIGN-SYSTEM.md](../LOST-MONSTER-DESIGN-SYSTEM.md)
**Slop Test:** [.ai/slop-test.md](../slop-test.md)
...
**Format:**
```
GAFFER: Last session shipped services page redesign (SOFAX: 87, TERRX: pass).
Open debt: projects page SOFAX dropped to 79 — needs polish.
...
2. **EXTRACT** signals → `touches-db`, `touches-ui`, `touches-api`, `marketing-page`, `mobile-relevant`, `conversion-critical`, `multi-file`, `new-entity`, `has-empty-states`
...
- **Building** — Which execution workers run during the build (CRUDX, DEMX, MAPX, UXPATX)
- **Review** — Who audits the output during BULLETPROOF (SOFAX, AIDAX, NIGELX, PIXLX, CONSX — parallel)
- **Sign-off** — Who runs last before presenting to James (TERRX, AUDIX, HARDX → GAFFER SIGN-OFF)

**Format:**
```
GAFFER: Projects page redesign — here's the crew:
  Planning:  CODAX (light — scope the change) → PETRAX (validate plan)
  Build:     UXPATX patterns for the portfolio grid
  Review:    SOFAX (design), NIGELX (usability)
  Sign-off:  TERRX (tests) → GAFFER SIGN-OFF
  Note:      This touches the services section — check existing
             patterns in app/ first (CONSX-style).
```
...
- Lost Monster has no admin app — skip admin-related routing. All pages are marketing/public pages

**Design Guide Loading (MANDATORY when `touches-ui` signal present):**

When the task involves ANY UI work, the Gaffer MUST read [.ai/LOST-MONSTER-DESIGN-SYSTEM.md](../LOST-MONSTER-DESIGN-SYSTEM.md) and extract the relevant constraints into the crew sheet notes. Workers can't follow rules they don't know exist. The Design Guide is the source of truth — not memory, not assumptions.

**Core Design System — Bold Personal Brand:**
Lost Monster follows a dark, dramatic personal brand aesthetic. Every page uses dark gradient backgrounds (`bg-gradient-to-br` with colour-specific gradients like `from-neutral-900 via-stone-900 to-neutral-900`), dynamic user-selectable accent colours (blue, teal, orange, purple, green), massive typography (up to 128px for hero headlines), and glassmorphism cards (`bg-white/5 backdrop-blur-md border rounded-xl`). This is the governing principle behind every page. Content lives on dark gradients with glass cards. No light backgrounds. No corporate aesthetic. No exceptions.

**Semantic Structure (MANDATORY for content-heavy pages):**
Bold Personal Brand is NOT "slap everything in one dark section." Content structure must reflect the semantic meaning of the page. Before building, **analyse the content**: identify distinct topics, content type changes (hero → metrics → services → testimonials → CTA), and natural reading breakpoints. Each section should be visually distinct — alternating between full-bleed dark gradient sections with different treatments. Never one continuous dark blob. The breathing room between sections signals "new topic" to the reader — losing that signal defeats the entire design system.

| What to Extract | Where in Design Guide |
|----------------|----------------------|
| **Bold Personal Brand principle** | "Core Design Principles" section — dark, dramatic, personal |
| **Dynamic colour system** | "Color System" section — 5 user-selectable accent colours |
| **Typography scale** | "Typography System" section — 128px heroes, Outfit + Inter |
| **Glassmorphism cards** | "Component Patterns" section — bg-white/5 backdrop-blur-md |
| **Grid pattern texture** | "Technical Subtlety" section — SVG grid background |
| **Colour application** | "Color Application" section — backgrounds, text, interactive |
| **Voice & tone** | "Voice & Tone Guidelines" — first person, direct, no jargon |
| **Metric cards** | "Component Patterns" — 4 key stats with glassmorphism |
| **Hero design** | Full viewport, massive headline, two CTAs |
| **Spacing** | Hero `h-screen`, sections `py-20 md:py-32` |
| **CTAs** | Accent colour background, glow shadow effect |
| **Icons** | Lucide React only, specific sizes per context |
| **AIDAX quality bar** | Must score 35+/40 to ship |
| **AI Slop Test** | Provenance Rule + 10 Red Flags from [.ai/slop-test.md](../slop-test.md) |

The crew sheet notes should include a **Design Constraints** block (not just brand constraints — the full design system):

```
GAFFER: Services page redesign — here's the crew:
  ...
  Design constraints (from Design Guide):
    - System: Bold Personal Brand — dark gradient backgrounds, dynamic accent colour. Always
    - Semantic structure: Analyse content FIRST. Distinct sections per topic. Never one continuous blob
    - Content-heavy pages: Split by topic (each service, each case study, FAQ sections, CTAs = separate sections)
    - Section test: 800+ words or 2+ topics in one section → must split. No lazy single-section pages
    - Backgrounds: Dark gradients (from-neutral-900 etc.), glassmorphism cards (bg-white/5 backdrop-blur-md)
    - Colour: Dynamic accent from 5 user-selectable options (blue, teal, orange, purple, green)
    - Typography: Outfit headings, Inter body, 128px hero headlines, tracking-tighter
    - Cards: bg-white/5 backdrop-blur-md border rounded-xl, border at accent/20
    - Hover: hover:bg-white/10 transition-all
    - CTAs: accent colour bg, glow shadow effect (boxShadow: 0 20px 60px -15px)
    - Voice: First person "I", direct, specific metrics, no jargon, no corporate speak
    - No light backgrounds on marketing. No "we" voice. No corporate patterns
    - AIDAX: must score 35+/40 to ship
```

**This is not optional.** If DEMX generates a light-background variation and recommends it as the winner, the Design Guide constraints should have been loaded into the crew sheet so DEMX would know light backgrounds violate the brand before generating variations.
...
|----------------|-------------------|
| Marketing page/component | NIGELX + AIDAX + SOFAX (SOFAX includes Dimension 11 Brand Compliance — non-negotiable for all pages) |
| Mobile-affected change | + PIXLX |
| Page with empty/loading states | + PIXLX |
| API endpoint | TERRX |
| Conversion-critical (enquiry, contact, CTA) | AIDAX (non-negotiable) |
...
**Format:**
```
GAFFER: This is a marketing projects page with empty states.
Mandatory: NIGELX, AIDAX, SOFAX, PIXLX, TERRX.
INSPX: Loading spec from inspections/projects.md — 4 checkpoints, 2 viewports.
...
Checking... PIXLX wasn't called when the services page was last shipped.
...
This gate exists because the Gaffer once skipped the review crew and presented unscored visual work directly to James. Basic failures that SOFAX and CONSX would have caught in seconds. Never again.
...
```
James: "Full Gaffer build — add a case studies section to the projects page"
...
PHASE 2: BUILD
+-- Gaffer determines approach: Full-stack (with API) or frontend-only
+-- If full-stack: DB -> Types -> API -> Components -> Integration
...
| Mobile considerations | All Lost Monster pages are user-facing — PIXLX always checks responsive behaviour |
...
```
GAFFER: FULL BUILD — Case Studies Section
---------------------------------------------

PHASE 1: PLANNING
  CODAX: Plan complete — 2 milestones, 8 todos
  PLANX: Execution blueprint written
  PETRAX: All todos atomic

PHASE 2: BUILD
  Frontend-only: New section component + data + integration
...
- **Projects page SOFAX regression** (flagged YYYY-MM-DD)
  Score dropped from 84 -> 79. Needs dedicated polish pass.
  Affected: card spacing, glassmorphism border alignment on mobile.

- **Aida coverage gap** (flagged YYYY-MM-DD)
  3 pages shipped without conversion check in last week.
  Action: Run AIDAX on projects, services, testimonials.

## Resolved

- **Mobile services overflow** (flagged YYYY-MM-DD, resolved YYYY-MM-DD)
  Fixed in services rebuild. Pixie now mandatory for services page changes.
```
...
- **SOFAX spacing dimension** (YYYY-MM-DD)
  Was scoring 10/12 on pages with 2px inconsistencies between sections.
...
- **YYYY-MM-DD: Contact form conversion dropped 15%**
...
**What happens:**
1. The specific violation is logged to [.ai/slop-test.md](../slop-test.md) Lessons Learned table (date, what slipped, which red flag)
...
GAFFER FEEDBACK: Slop caught by SOFAX Dim 11.
  Violation: Light background used as mid-page content section (Red Flag #2: orphan pattern)
  Generated by: DEMX (V4 Light Canvas)
  Logged to: .ai/slop-test.md lessons table
...
3. The pattern is also added to [.ai/slop-test.md](../slop-test.md) Red Flags if it's a new pattern not already covered

**Format:**
```
GAFFER FEEDBACK: DEMX Brand Gate disqualified V3.
  Reason: bg-white mid-page section (approved backgrounds violation — Lost Monster is dark only)
...
2. If the conflict reveals a **rule that should exist but doesn't** in the Design Guide, the Gaffer proposes an addition to [.ai/LOST-MONSTER-DESIGN-SYSTEM.md](../LOST-MONSTER-DESIGN-SYSTEM.md) for James's approval
...
GAFFER FEEDBACK: CONSX found undocumented pattern rule.
  Conflict: Projects page uses backdrop-blur-lg but services page uses backdrop-blur-md — no rule specifies blur level.
  Proposal: Add to Design Guide -> "All glassmorphism cards use backdrop-blur-md. No blur variants on marketing pages."
...
**Format:**
```
GAFFER FEEDBACK: AIDAX scored 72/100 on services page.
  Weakest: Desire (5/10) — pricing section doesn't create urgency.
  Debt logged: "Services page conversion — Desire dimension weak"
...
- Title lines (`— Lost Monster Edition`)
- `## Lost Monster Context` sections in every worker file
- Worker scenarios (NIGELX's task, AIDAX's conversion flow, SOFAX's design references)
- Entity examples (projects/services/testimonials)
- Tech stack references (Next.js/Neon PostgreSQL/Vercel)
...
```
James: "Gaffer: onboard from .ai/DOMAIN-KNOWLEDGE.md"

The Gaffer reads the PRD and extracts:

1. PROJECT IDENTITY
   +-- Project name: Lost Monster
   +-- One-line description: Personal brand development agency
   +-- Target users: Small business owners like Dave (-> becomes NIGELX's scenario)
   +-- Core user journey: Browse -> See proof -> Contact (-> becomes AIDAX's conversion flow)
   +-- Design aspirations: Bold, dark, personal (-> becomes SOFAX's reference points)

2. DOMAIN MODEL
   +-- Core entities: Projects, Services, Testimonials (-> becomes examples)
   +-- Relationships between entities
   +-- Key user actions: View portfolio, check services, read testimonials, make contact
   +-- Business rules (-> becomes PIXLX's edge cases)

3. TECH STACK
   +-- Framework: Next.js 15 App Router (-> updates all code examples)
   +-- Database: Neon PostgreSQL (-> updates CRUDX layer 1)
   +-- Hosting: Vercel (-> updates TERRX)
   +-- Testing framework (-> updates TERRX)
   +-- File structure: Single app/ directory (-> updates all path references)

4. CONVERSION GOALS
   +-- Primary CTA: "Start Your Project" / contact (-> becomes AIDAX's key action)
   +-- Conversion funnel: Hero -> Metrics -> Services -> Projects -> Testimonials -> Contact
   +-- Success metrics: Enquiry form submissions

Then The Gaffer rewrites EVERY worker file:

REWRITING WORKERS:
  GAFFER.md   — Updated routing examples (projects, services, testimonials)
  ...
  PLANX-SEO-GEO.md — Updated keywords, competitors
...
LOGGING TO EVOLUTION:
  .ai/gaffer/evolution.md — "Onboarded to Lost Monster" entry added

DONE. All workers rewritten for Lost Monster.
...
GAFFER: No PRD provided. I need the basics to onboard:
  ...
  4. What are the main entities/objects? (e.g. projects, services, testimonials)
  5. Tech stack? (framework, DB, hosting, testing)
  6. Design inspiration? (e.g. "Bold, dark, personal brand")
```
