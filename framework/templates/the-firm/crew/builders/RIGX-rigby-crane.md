...
**RIGX for [PROJECT]** understands:
- **Monorepo structure** — `[APP-PUBLIC]`, `[APP-ADMIN]`, `packages/*`
- **[DATABASE]** with `[DB-DRIVER]` driver
- **[AUTH-METHOD]** for authentication
...
- [ ] Path aliases working (`@/` or `@[PROJECT]/`)
...
- [ ] Database provisioned ([DATABASE]) — instance exists and is accessible
...
- [ ] ORM or query client configured and connected (`[DB-DRIVER]`)
- [ ] Database client importable: `import { db } from '@[PROJECT]/shared/lib/db/client'`
...
- [ ] Auth provider configured ([AUTH-METHOD]) — API keys in `.env.local`
...
- [ ] External services connected:
  - [ ] Email service ([EMAIL-SERVICE]) — can send a test email
...
- [ ] Hosting provider connected ([HOSTING-PROVIDER]) — project linked to repo
...
# Visit https://[DOMAIN] → app loads correctly
# Check https://[DOMAIN]/api/health → returns 200
```
...
```
RIGBY INFRASTRUCTURE REPORT: [PROJECT]
═══════════════════════════════════════════════════
...
**Framework Status:** Template (customise [DATABASE], [AUTH-METHOD], [OBJECT-STORAGE], [HOSTING-PROVIDER] for project)
