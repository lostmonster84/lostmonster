# STANX — Security Audit Framework — [PROJECT] Edition
...
**STANX for [PROJECT]** understands:
- **Monorepo structure** — `[APP-PUBLIC]`, `[APP-ADMIN]`, `[APP-SUPERADMIN]`, `[APP-API]`
- **[AUTH-METHOD]** — Authentication provider, session management, role-based access
- **[DATABASE]** — Data layer with [entity-primary], [entity-secondary], [entity-users]
- **API routes** — REST endpoints, server actions, middleware
- **[HOSTING-PROVIDER]** deployment — Production environment, environment variables
...
When building features for [PROJECT], Stan expects:
