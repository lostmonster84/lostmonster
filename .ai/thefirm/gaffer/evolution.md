...
**What changed:**
- **AIDAX Synthesis Step (required)** in `crew/reviewers/AIDAX-aida-sterling.md`. After scoring N DEMX variants, AIDAX MUST propose Variant N+1 that takes each winner's strongest dimension and explicitly kills each loser's failure mode. Re-scored on the same 0-100 rubric, must beat all N originals. Includes presentation format, rules (no Frankenstein, explicit kills, brand-aligned, re-score required), and skip conditions (one variant already 90+, all sub-60, single-variant task). Generalised to any N, typically 3 or 5.
- **Em-dash ban (framework-wide)** in `crew/builders/WORDX-wren-calloway.md`. New row in the AI Slop Detection table. The long-dash character (U+2014) is banned across all copy, UI, docs, comments, commit messages, and agent replies. Grep-checkable. Gaffer blocks sign-off on any occurrence in changed files.
- **Em-dash sweep** across AIDAX, WORDX, DEMX. All upstream framework files now contain zero em-dash characters.

**Why:** Synthesis, forced-choice between N DEMX variants loses signal. Scoring on its own is a ranking exercise; synthesis is how scoring becomes a design improvement. A CTA-redesign round produced three variants scoring 85 / 78 / 75 and the user asked "can we take the best of all and make the final?". The resulting synthesis variant scored 91 and shipped. That move was retrospectively recognised as the correct default, not a one-off.

Em-dashes, the long-dash character is a textbook AI-generated-text tell. Repeated user feedback flagged it as AI-slop. Having the character appear in framework files that propagate to every project compounds the problem.

**How this prevents recurrence:**
- Any future DEMX round automatically produces a Variant N+1 synthesis proposal before the user is asked to pick.
- Any file touched by any worker is grep-checked for the em-dash character before sign-off. Gaffer blocks CLEARED on occurrence.

**Files changed:**
- `.ai/thefirm/crew/reviewers/AIDAX-aida-sterling.md`, new Design Spec Feedback Loop section with Synthesis Step subsection, em-dash sweep
- `.ai/thefirm/crew/builders/WORDX-wren-calloway.md`, new em-dash row in AI Slop Detection table, em-dash sweep
- `.ai/thefirm/crew/builders/DEMX-dex-carousel.md`, em-dash sweep

---

## v3.17, Write-Path Verification Gate + `state-mutating-ui` Signal (2026-04-20)

**Category:** Minor - new Foreman checkpoint + new Smart Routing signal + PIXLX scope widening

**What changed:**
- **Foreman's checklist: 9 → 10 points.** New Point 8 "Write-Path Verification" - for any build that ships a POST/PUT/DELETE endpoint, the action must be exercised live (happy path + empty-state variant + failure path) before CLEARED is issued. Explicit failure mode named: read-only Eyes On + cold build + tsc clean is NOT sufficient for state-mutating UI. Code-read reasoning that it "should work" is called out as insufficient evidence.
- **New Smart Routing signal `state-mutating-ui`** - detected on any form/button that POSTs/PUTs/DELETEs. Added to Step 3 Extract table + Step 4 Signal Boosters (+3 to PIXLX, TESTX, STANX).
- **PIXLX mandatory trigger widened** - now mandatory when `mobile-relevant` OR `state-mutating-ui` OR `has-empty-states`. "Desktop-only is NOT a sufficient skip reason" is explicit in the mandatory overrides table. Covers edge cases across all dimensions, not just viewport.
- **PIXLX playbook updated** - new "Not Just Mobile" scope section. Covers write-path edge cases, empty-state forms, nullable-FK pre-fills, optional fields. Historical trigger (v3.17 incident) named.
- **GAFFER.md** - inline Smart Routing updated: Step 3 signal list + Step 6 mandatory overrides. All "9-point" references updated to "10-point".

**Why:** A state-mutating Edit modal shipped through the full chain (cold build + tsc clean + Review Card + Frank CLEARED + Gaffer APPROVED) and 500'd on the first real click. Root cause: form pre-filled `""` into a nullable UUID column; endpoint never exercised against the empty-state variant. PIXLX was skipped with rationale "superadmin is desktop-primary; mobile not required" - the skip conflated PIXLX's scope (edge cases across all dimensions) with viewport-level mobile checks. Frank's 9-point had no explicit write-path check; "actionable" was interpreted as "renders and clicks" rather than "action completes end-to-end." Full trace in `calibration.md`.

**How this prevents recurrence:**
- Any future build with a write endpoint triggers `state-mutating-ui` signal → PIXLX is mandatory → edge-case audit covers empty-state variants.
- Frank's Point 8 forces a live round-trip before CLEARED - happy path AND empty-state variant both exercised.
- PIXLX playbook explicitly scopes "nullable FK pre-fills" as in-territory.

**Files changed:**
- `.ai/thefirm/crew/FOREMAN.md` - 9-point → 10-point, new Point 8 Write-Path Verification
- `.ai/thefirm/PROTOCOL.md` - Step 3 (new signal), Step 4 (new booster row), Step 7 (PIXLX trigger widening)
- `.ai/thefirm/crew/GAFFER.md` - Smart Routing Step 3 + Step 6 sync, all 9-point references updated
- `.ai/thefirm/crew/reviewers/PIXLX-pixie-edge.md` - scope section added
- `.ai/thefirm/gaffer/calibration.md` - full James Rejection Trace logged
