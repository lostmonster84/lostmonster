# APEX Framework

> **All-Protocol EXecution - The Ultimate Meta-Framework**
>
> One command to activate all frameworks in orchestrated sequence for Lost Monster features.

<!-- ONBOARD:START -->
| Token | Value | Source |
|-------|-------|--------|
| `[PROJECT]` | Lost Monster | CLAUDE.md |
| `[PROJECT-DOMAIN]` | lostmonster.io | CLAUDE.md |
| `[ENTITY-PRIMARY]` | Projects | CLAUDE.md |
| `[ENTITY-SECONDARY]` | Case Studies | CLAUDE.md |
| `[ENTITY-USERS]` | Clients | CLAUDE.md |
| `[DATABASE]` | Neon PostgreSQL | CLAUDE.md |
| `[HOSTING-PROVIDER]` | Vercel | CLAUDE.md |
| `[BRAND-PRIMARY]` | #06B6D4 (teal) | CLAUDE.md |
| `[BRAND-BG]` | Dark/black backgrounds | CLAUDE.md |
| `[BRAND-DARK]` | Dark theme with glassmorphism | CLAUDE.md |
| `[MAP-SERVICE]` | | |
| `[APP-PUBLIC]` | website/ (port 3000) | CLAUDE.md |
| `[APP-ADMIN]` | dashboard/apps/web/ (port 3001) | CLAUDE.md |
| `[APP-API]` | Next.js API routes (website/app/api/ + dashboard/apps/web/src/app/api/) | CLAUDE.md |
| `[DESIGN-GUIDE-PATH]` | website/.ai/LOST-MONSTER-DESIGN-SYSTEM.md | CLAUDE.md |
| `[TEST-PERSONA]` | Graduate Grace | CLAUDE.md |
<!-- ONBOARD:END -->

---

## Lost Monster Context

**APEX for Lost Monster** orchestrates the full builder stack to deliver production-ready features across the Lost Monster monorepo. It coordinates database migrations (Neon PostgreSQL), API routes, admin interfaces (dashboard on port 3001), and public-facing pages (website on port 3000) — all wired together in a single command.

Lost Monster is a personal brand + agency site built with Next.js 15, Tailwind CSS, and a dark-themed design system with 5 dynamic colour themes. APEX ensures every feature ships with Graduate Grace-friendly UX, SOFAX-audited design (93+/110), and full brand compliance.
---

## What is APEX?

**APEX** orchestrates all frameworks in perfect sequence to deliver production-ready Lost Monster features from a single command.

### The APEX Promise

**You say:** `APEX: [feature description]`

**You get:**
- ✅ Complete backend (Neon PostgreSQL tables + API routes + Types)
- ✅ Admin interface (dashboard/apps/web/ (port 3001))
- ✅ Public UI with AIDA conversion structure
- ✅ Mobile-native experience (gestures + touch)
- ✅ SOFAX-audited design (93+/110 score)
- ✅ Production-ready code

---

## When to Use APEX

### ✅ Use APEX For

**Major Lost Monster Features:**
- Projects management system (CRUD + display)
- Search & filters with map view
- Clients dashboard sections
- Case Studies management system
- Landing pages
- Projects detail pages

**Perfect APEX Candidates:**
- "Search with filters and map toggle"
- "Management dashboard"
- "Lead inbox with responses"
- "Landing pages for key areas"
- "Comparison feature"
- "Saved items / favourites system"

### ❌ Don't Use APEX For

**Use Individual Frameworks Instead:**
- Bug fix → Just fix it
- Card redesign → DEMX
- Search UX audit → SOFAX
- Mobile enhancement → TUCHX
- New API endpoint → CRUDX

---

## The APEX Workflow

```
┌─────────────────────────────────────────────────────────────────┐
│                       APEX COMMAND RECEIVED                      │
│               "APEX: [feature description]"                      │
└─────────────────────────────────────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 1/6: INTERPRET                                             │
├─────────────────────────────────────────────────────────────────┤
│ • Parse feature request                                         │
│ • Identify scope and entities involved                          │
│ • Determine scope                                               │
│ • Generate comprehensive requirements                           │
├─────────────────────────────────────────────────────────────────┤
│ Quality Gate: ☑ Lost Monster domain requirements clear             │
└─────────────────────────────────────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 2/6: PLAN (CODAX)                                         │
├─────────────────────────────────────────────────────────────────┤
│ • Context: Current Lost Monster state (website/ (port 3000), dashboard/apps/web/ (port 3001))  │
│ • Objective: Feature goal                                       │
│ • Details: Entities and relationships involved                  │
│ • Acceptance: PRD alignment, domain rules                       │
├─────────────────────────────────────────────────────────────────┤
│ Quality Gate: ☑ User has approved plan                          │
└─────────────────────────────────────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 3/6: BUILD BACKEND (CRUDX)                                │
├─────────────────────────────────────────────────────────────────┤
│ Layer 1: Neon PostgreSQL schema                                      │
│ Layer 2: TypeScript types                                       │
│ Layer 3: API routes (Next.js API routes (website/app/api/ + dashboard/apps/web/src/app/api/))                                 │
│ Layer 4: Admin UI (dashboard/apps/web/ (port 3001))                                 │
│ Layer 5: Components                                             │
│ Layer 6: Integration (nav, status badges, indicators)           │
├─────────────────────────────────────────────────────────────────┤
│ Quality Gate: ☑ All 6 layers complete                           │
│               ☑ Auth guards applied                             │
│               ☑ Type generation working                         │
└─────────────────────────────────────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 4/6: STRUCTURE CONTENT (AIDAX)                            │
├─────────────────────────────────────────────────────────────────┤
│ • Attention (0-25): Hero content, key badges                    │
│ • Interest (0-25): Key details, features                        │
│ • Desire (0-25): Lifestyle copy, trust signals                  │
│ • Action (0-25): Primary CTA, save, share                      │
├─────────────────────────────────────────────────────────────────┤
│ Quality Gate: ☑ Score 80+ for content                           │
└─────────────────────────────────────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 5/6: MOBILE-NATIVE (TUCHX)                                │
├─────────────────────────────────────────────────────────────────┤
│ Gallery:                                                        │
│ • Swipe with snap                                               │
│ • Pinch-to-zoom                                                 │
│ • Share via native sheet                                        │
│                                                                 │
│ Map Experience:                                                 │
│ • Pinch-zoom on map                                             │
│ • Tap marker → preview                                          │
│ • Cluster expand gesture                                        │
│                                                                 │
│ Search & Filters:                                               │
│ • Bottom sheet filter panel                                     │
│ • Pull-to-refresh                                               │
│ • Infinite scroll results                                       │
├─────────────────────────────────────────────────────────────────┤
│ Quality Gate: ☑ 60fps verified                                  │
│               ☑ Touch targets 44px+                             │
└─────────────────────────────────────────────────────────────────┘
                               ↓
┌─────────────────────────────────────────────────────────────────┐
│ STAGE 6/6: AUDIT QUALITY (SOFAX)                                │
├─────────────────────────────────────────────────────────────────┤
│ Audit:                                                          │
│ 1. Typography: Hierarchy, titles                                │
│ 2. Spacing: Grid rhythm, spacing                                │
│ 3. Touch Targets: Buttons, interactive elements                 │
│ 4. Information Density: Cards, results                          │
│ 5. Visual Hierarchy: Content prominence                         │
│ 6. Sophistication: Integration, smooth transitions              │
│ 7. Consistency: Styles across views                             │
│ 8. Accessibility: Screen reader support                         │
│                                                                 │
│ Brand Gate (mandatory sub-step):                                │
│ 9. Provenance Rule: Every colour, border, shadow, gradient      │
│    must exist on another live Lost Monster page. Orphan = reject.  │
│ 10. Approved Backgrounds: Use approved palette only.            │
│ 11. AI Slop Red Flags: No thick coloured borders, gratuitous    │
│     gradients, decorative flourishes, or orphan patterns.       │
│ Ref: website/.ai/LOST-MONSTER-DESIGN-SYSTEM.md + AI Slop Test (10 Red Flags)         │
│ If Brand Gate fails → reject variation, rebuild before proceed  │
├─────────────────────────────────────────────────────────────────┤
│ Quality Gate: ☑ Score 85+ achieved                              │
│               ☑ Benchmarked against industry leaders             │
│               ☑ Brand Gate passed (Provenance + no AI Slop)     │
└─────────────────────────────────────────────────────────────────┘
                               ↓
                         ┌──────────┐
                         │  LAUNCH  │
                         │    ✅     │
                         └──────────┘
```

---

## APEX Modes

### Full APEX (Default)
**Command:** `APEX: [feature]`
**Executes:** All 6 stages
**Use for:** Major features

### Partial APEX Modes

#### APEX-STATIC
**Command:** `APEX-STATIC: [static page]`
**Skips:** CRUDX (Stage 3)
**Use for:** Static content pages (About, How it Works, Contact)

#### APEX-DESKTOP
**Command:** `APEX-DESKTOP: [dashboard feature]`
**Skips:** TUCHX (Stage 5)
**Use for:** Dashboard features (desktop-first)

#### APEX-BACKEND
**Command:** `APEX-BACKEND: [backend feature]`
**Skips:** AIDAX, TUCHX, SOFAX (Stages 4-6)
**Use for:** Backend automation (emails, webhooks)

#### APEX-FRONTEND
**Command:** `APEX-FRONTEND: [frontend feature]`
**Skips:** CRUDX (Stage 3)
**Use for:** Frontend features using existing APIs

---

## Lost Monster Quality Targets

| Stage | Framework | Target |
|-------|-----------|--------|
| 4 | AIDAX | 80+ (content conversion) |
| 5 | TUCHX | 60fps + 44px+ (gestures) |
| 6 | SOFAX | 93+/110 (industry-quality UX) |
| 6 | Brand Gate | Provenance Rule passed, no AI Slop red flags |

---

## Trigger Examples

```
APEX: search with map view
APEX: detail page with enquiry form
APEX: dashboard with management
APEX: favourites system with saved searches
APEX: landing page for [area]
APEX: inbox with responses
APEX-DESKTOP: analytics dashboard
APEX-STATIC: how it works page
APEX-BACKEND: notification system
```

---

## Quick Reference

### Commands
- `APEX: [feature]` - Full execution (all 6 stages)
- `APEX-STATIC: [feature]` - Skip backend
- `APEX-DESKTOP: [feature]` - Skip mobile
- `APEX-BACKEND: [feature]` - Backend only
- `APEX-FRONTEND: [feature]` - Frontend only

### Quality Targets
- AIDAX: 80+ (content)
- SOFAX: 93+/110 (industry-quality)
- TUCHX: 60fps (gestures)
- CRUDX: All 6 layers complete

### Lost Monster Entities
- **Projects** - Primary content entities
- **Case Studies** - Secondary entities (enquiries, leads)
- **Clients** - User/account entities

---


---

## Supplements

Before starting work, check for a relevant supplement in `builders/supplements/`:

| Job Type | Supplement | Created |
|----------|-----------|---------|

If a supplement exists for this job type, **read it before starting work**.
It contains researched patterns from real-world examples.

If no supplement exists and the job type is unfamiliar, flag it — SCOUTX may need to research first.


**Framework Status:** Generic
**Last Updated:** March 2026
**Version:** 3.0
