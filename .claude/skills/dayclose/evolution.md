...
## 2026-05-16 — Firm v4.6.2 Rules 18 + 19 + Risk Scan + Telemetry Block

**Caught:** All four upgrades shipped clean — local commit `64e516b` (17 files, +856/-50) + upstream `f253927` (7 files, +361/-44). Generalisation Gate caught a real `Lost Monster → [PROJECT]` reverse-replace miss in upstream GAFFER.md (2 instances onboarded during /sync); fixed before push. README worker-count audit passed (1 researcher + 7 planners + 9 builders + 7 reviewers + 10 checkers = 34, matches all claims; line 149 "30 worker context sections" → 34 fixed in same push). Stale Frank point-count refs in upstream PROTOCOL.md ×2 ("currently 14") + CLAUDE-TEMPLATE.md ×1 ("currently 16") co-fixed to "currently 18" — same drift pattern as previous /gaffer SKILL.md drift, now closed. Missing v4.6.1 CHANGELOG entry (yesterday's `chore(v4.6.1)` push didn't update CHANGELOG) backfilled in same v4.6.2 entry. Pre-commit risk scan fired correctly during local commit (7 self-referential signals, none false-negative). `.playwright-mcp/` exploration artifacts cleaned. `.agents/` + `**/.next/` + `.playwright-mcp/` added to .gitignore. AGENTS.md committed (Codex peer to CLAUDE.md, legitimate repo artefact).

**Missed:** Pre-existing upstream contamination in GAFFER.md design-constraints example block (LOST-MONSTER-DESIGN-SYSTEM.md + btn-brand-teal #1A5F5F + btn-brand-midnight #1A1940 + malformed `docs/website/.ai/LOST-MONSTER-DESIGN-SYSTEM.md.md` in PROTOCOL.md ×2) — surfaced by Generalisation Gate's whole-file scan but pre-existing (already on upstream HEAD before today). Per gate spec these are debt-loggable not blocking. Debt-logged in upstream v4.6.2 CHANGELOG entry. Separate thefirm-repo genericisation session needed. Also missed: the `/firm` skill's "always update changelog" rule was honoured for v4.6.0 but missed for v4.6.1 (caught only because today's check found the gap) — TRAINX should treat as `/firm` skill calibration miss.

**Friction:** Bash sanity-check regex in pre-commit staging audit was buggy (used `$` anchor inside group, causing all expected paths to surface as "unexpected" — visual inspection confirmed clean). Edit tool repeatedly required Read-first calls even on files already grep'd via Bash — 6 wasted Edit attempts in Phase A. Generalisation Gate script had bash arithmetic noise (`integer expression expected: 0\n0`) from `grep -Fc` returning multi-line output; non-fatal but messy. v4.6.2 introduced Rule 19 which then immediately ran on v4.6.2's own build (all 4 sub-checks PASS — Self-Compliance Gate dogfooded itself on its own deployment session) — recursive self-application worked but adds ~45 min of process overhead per material framework change. v4.6.3 should monitor whether the cost stays proportional.

**User overrode:** None. User asked one targeted self-awareness question ("is this going to improve what we've already got?") which surfaced Rule 19 over-scoping and led to the material/trivial carve-out — saved before push. This is the model of mid-session course-correction the framework should support; not an override, a refinement.
