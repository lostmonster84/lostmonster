...
## Step 5d: Scaffold Forensic-Block Infrastructure (MANDATORY)

The `/wrap` and `/dayclose` skills require a set of files at the project root to generate, validate, and enforce forensic blocks. These ship in `~/Projects/thestack/template/` and reach NEW projects via `setup.sh`. Existing projects upgraded via `/sync` would otherwise hit missing-file errors on their first `/wrap` after the upgrade, OR ship commits without the validation hook firing. This step closes that gap.

The full set of forensic-block scaffold files:

| Project path | Stack template path | Purpose |
|---|---|---|
| `scripts/forensic-log.ts` | `template/scripts/forensic-log.ts` | Generator that reads `subsystems.json` + `git diff` and outputs the block |
| `subsystems.json` | `template/subsystems.json` | Subsystem-to-glob map; drives Subsystems/Risk-surface auto-fill |
| `scripts/lint-subsystems.ts` | `template/scripts/lint-subsystems.ts` | Validator (dupe globs, missing fields, uncategorised drift). Wired into `/dayclose` Step 1b sub-step 8 |
| `scripts/install-hooks.sh` | `template/scripts/install-hooks.sh` | Idempotent: sets `core.hooksPath = .githooks`, chmods every hook |
| `.githooks/commit-msg` | `template/.githooks/commit-msg` | Validates Verified/Deferred fields; rejects `<TODO:>` placeholders |
| `.githooks/post-merge` | `template/.githooks/post-merge` | Checks The Firm for updates on git pull (lives in `.githooks/` so `core.hooksPath` activates it) |
| `.github/pull_request_template.md` | `template/.github/pull_request_template.md` | PR template with embedded forensic block fence (so squash-merge bodies inherit validation) |

For each file, scaffold-if-missing (idempotent):

1. **For each row above, in order:**
   - If the project file exists locally, skip silently
   - If the corresponding Stack template file does NOT exist, skip silently (Stack hasn't been bumped to the full forensic-block protocol yet on this machine - older Stack)
   - Otherwise:
     - Create the parent directory if missing (`mkdir -p`) - includes `scripts/`, `.githooks/`, `.github/`
     - Copy from `~/Projects/thestack/template/<path>` to `<project-root>/<path>`
     - Preserve executable bit on `commit-msg`, `post-merge`, `install-hooks.sh` (`chmod +x`)
     - Report: "Scaffolded `<path>`"

2. **After the file copies, wire up the hooks (only if at least one of `.githooks/*` was newly scaffolded):**
   - Run `bash <project-root>/scripts/install-hooks.sh` (idempotent: sets `core.hooksPath = .githooks` if not already set, chmods all hooks)
   - Report: "Wired core.hooksPath -> .githooks"
   - If hook install fails (e.g. the project root isn't a git repo yet), report a warning but don't halt the sync

3. **Order matters:** scaffold the scripts + manifest BEFORE the hooks. The hooks reference scripts (`forensic-log.ts` is referenced by the PR template; `lint-subsystems.ts` is referenced by `/dayclose`). The order in the table above is the safe order.

4. **Never overwrite.** If a file already exists, the project owns it - leave it alone, even if the upstream version is newer. Project-specific manifests, customised hooks, and tweaked PR templates are valuable; replacing them would lose customisation. The only exception is the install-hooks.sh wiring step (point 2), which is idempotent by design.

5. **Report nothing if everything already existed.** Quiet success is the default. A good sync prints zero lines from this step.

The skeleton manifest (`template/subsystems.json` in the Stack) has 9 generic subsystems (framework, config, infra, auth, database, ui-primitives, design-system, tests, docs). Project-specific subsystems get added incrementally as the manifest is exercised. The "uncategorised" bucket in forensic-log output is the signal to extend.
